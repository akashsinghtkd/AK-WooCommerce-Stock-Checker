 ...
 */
define( 'AKWSC_PLUGIN_FULL_NAME', 'AK WooCommerce Stock Checker' );

/**
 * Currently plugin name.
 * Start at version 1.0.0
 */
define( 'AKWSC_NONCE_KEY', 'stock_check_nonce' );

/**
 * Current plugin text domain.
 * Start at version 1.0.0 and use SemVer - https://semver.org
 */
define( 'AKWSC_TEXT_DOMAIN', 'akwsc' );
...
function akwsc_woocommerce_not_available_admin_notice() {
    ...
    // Display messages based on WooCommerce status
    if ( ! $woocommerce_installed ) {
        $message = Akwsc_Plugin_Messages::woocommerce_not_installed();
    } else {
        $message = Akwsc_Plugin_Messages::woocommerce_not_activated();
    }
